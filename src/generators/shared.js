/**
 * Shared HTML fragment builders used by all three report generators.
 */
import { fmtDate } from '../utils/date.js';

// ── Shared inline CSS (injected into every generated report) ────────────────
...
export function hbars(items, color, nameWidth = '110px') {
  if (!items.length) return '<p style="font-size:11px;color:#999;font-style:italic;">No data for this period.</p>';
...
export function compRows(rows) {
  return rows.map((r, i) => {
    const d = r.delta;
    const change = d > 0 ? `<span class="ch-up">${d} ▲</span>`
                 : d < 0 ? `<span class="ch-dn">${Math.abs(d)} ▼</span>`
                 :          `<span class="ch-fl">0 —</span>`;
    return `<tr>
  <td class="no">${i + 1}</td>
  <td>${r.type}</td>
  <td class="num">${r.lw}</td>
  <td class="num">${r.pw}</td>
...
// ── Page header ──────────────────────────────────────────────────────────────
export function pageHeader(title, reportDate, lwLabel, pwLabel, primary) {
  return `
...
// ── SLA / Compliance bar pair ────────────────────────────────────────────────
export function slaBars(passLabel, passCount, passPct, failLabel, failCount, failPct, accent) {
  return `
<div class="sla-row">
  <div class="sla-name">${passLabel}</div>
  <div class="sla-track"><div class="sla-fill" style="width:${passPct}%;background:${accent};"></div></div>
  <div class="sla-val">${passCount} <span style="color:${accent};">(${passPct}%)</span></div>
</div>
<div class="sla-row">
  <div class="sla-name">${failLabel}</div>
  <div class="sla-track"><div class="sla-fill" style="width:${failPct}%;background:#D9534F;"></div></div>
  <div class="sla-val">${failCount} <span style="color:#D9534F;">(${failPct}%)</span></div>
</div>`.trim();
}

// ── Bottom Chart.js script block ────────────────────────────────────────────
export function chartScript(pwLabels, pwData, lwLabels, lwData, pwColor, lwColor, tooltipSuffix = 'cases') {
  return `
<script>
const _bOpts = (color) => ({
  responsive: true,
  plugins: {
    ...
    tooltip: { callbacks: { label: ctx => \` \${ctx.raw} ${tooltipSuffix}\` } }
  },
  scales: {
    x: { ticks: { font:{size:10}, maxRotation:35, minRotation:20 }, grid:{display:false} },
    y: { ticks: { font:{size:10}, precision:0 }, grid:{color:'#EEE'} }
  }
});
new Chart(document.getElementById('pwChart'), {
  type: 'bar',
  data: {
    labels: ${JSON.stringify(pwLabels)},
    datasets: [{ label: '${tooltipSuffix}', data: ${JSON.stringify(pwData)}, backgroundColor:'${pwColor}', borderRadius:3, borderSkipped:false }]
  },
  options: _bOpts('${pwColor}')
});
new Chart(document.getElementById('lwChart'), {
  type: 'bar',
  data: {
    labels: ${JSON.stringify(lwLabels)},
    datasets: [{ label: '${tooltipSuffix}', data: ${JSON.stringify(lwData)}, backgroundColor:'${lwColor}', borderRadius:3, borderSkipped:false }]
  },
  options: _bOpts('${lwColor}')
});
</script>`.trim();
}
